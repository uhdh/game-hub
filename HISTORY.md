...
## 2026-08-04 · Claude Code · 수식 콤보 자동 배치·솔버 전면 재작성 (10만점 목표 달성, 미배포)
...
- **배포**: **하지 않았다.** game-hub에 커밋 `bd17312`까지 만들어 뒀지만 `git push`가 이 세션의 자동 승인
  정책에 막혔다. 다음 세션에서 `cd project/game-hub && git push origin master`를 실행하면 Vercel이 자동 배포한다.
  (원격 `01db574`는 이미 fast-forward로 병합해 둬서 추가 충돌은 없다.)

- **다음 작업 / 판단이 필요한 것**:
  1. 위 푸시 실행.
  2. **리더보드 정리 판단**: 오늘 로컬 검증 때문에 프로덕션 Supabase에 봇 점수가 올라갔다.
     `최혜선#21` 631,380점(id 133)이 현재 1위이고, `이진형#976` 18,546점(id 132), `허성범#719` 21점(id 130),
     `김유현#271` 0점(id 131)도 오늘 16:00 이후 테스트로 생긴 행이다. 실제 참가자 기록이 아니므로
     지우는 게 맞다고 보지만, 되돌릴 수 없어서 임의로 삭제하지 않고 남겨 뒀다.
  3. 개발용 컨트롤을 공개 배포본에 계속 둘지는 여전히 미결이다 (현재는 관리자 세션에서만 노출).
