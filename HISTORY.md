...
- **미결 / 확인 필요**:
  1. **`×(-10)` 시작값 해석이 사용자 의견과 다르다.** 사용자는 `10`이라고 했으나, 번들 `he()`는
     `e[0].value`를 그대로 쓰고 해당 타일의 `value`는 `-10`이라서 **현재 배포된 게임은 `-10`으로 계산한다**
     (브라우저에서 단독 클릭 시 `LIVE COMBO -10 = -10` 확인). 솔버를 `10`으로 바꾸면 게임의 실제 계산과
     어긋나 콤보 선언이 실패한다(2026-08-04 Codex 항목의 `+10` 시도가 0~1콤보였던 이유). 의도가
     "게임 규칙 자체를 10으로 바꾸는 것"이라면 `formula-combo` **번들 소스**(이 저장소에 없음)를 고쳐야 한다.
  2. 개발용 컨트롤을 공개 배포본에 계속 둘지 (현재는 관리자 세션에서만 노출).
